...
public class EventHandler {
	...
	public void handleStuurKoerier(StuurKoerierEvent event) {
		Response response = koerierService.stuurKoerier(event.getOrderId(), event.getKoerier(), event.getOntvanger(), event.getAfzender(), event.getAanmaakDatum(), event.isSpoed(), event.isExtern(), 
				event.getOrderStatus() == OrderStatus.OP_TE_HALEN ? true : false);
